...
def main():
	...
	workflow_name = None
	version = None
	# check_file_exists(opts.bowtie,'bowtie')
	# check_file_exists(opts.samtools, 'samtools')
	#print "start main"
	    #first option
        if opts.workflow:
            ...
            if not opts.output_directory:
                opts.output_directory = opts.input_directory + '/GF_output'
            ...
            ids,workflow_name,version,suffix = seqFileName.split(".", 3)
            files = fastq_files
        # second option
            ...
            if not opts.output_directory:
                opts.output_directory = os.path.dirname(opts.fastq_1) + '/GF_output'
    ...
    #===================#
    #print outdir
    gene_finder_functions.run_gene_finder(outdir,fasta_file,fastq_files,bowtie_options,workflow_name,version,ids,cut_off,minimum_coverage)
main()
